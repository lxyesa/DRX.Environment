    ...
    public static class JsonFile
    {
        ...
        public static void WriteToFile<T>(T obj, string filePath, bool append = false, JsonSerializerOptions? options = null)
        {
            ...
            try
            {
                ...
                var jsonString = JsonSerializer.Serialize(obj, options ?? DefaultOptions);
        ...
        public static void WriteJsonKey(string filePath, string? key, object value, JsonSerializerOptions? options = null)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(key))
                {
                    ...
                    {
                        // 将值序列化为 JSON 字符串
                        var valueJson = JsonSerializer.Serialize(value, options ?? DefaultOptions);
                        rootObject = JsonNode.Parse(valueJson) as JsonObject;
                    }
                }
                else
                {
                ...
                // 将 JSON 对象序列化为字符串
                var newJsonString = rootObject?.ToJsonString(options ?? DefaultOptions);
