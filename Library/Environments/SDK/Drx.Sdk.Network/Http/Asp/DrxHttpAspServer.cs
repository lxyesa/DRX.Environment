...
using HttpResponse = Drx.Sdk.Network.Http.Protocol.HttpResponse;
using HttpRequest = Drx.Sdk.Network.Http.Protocol.HttpRequest;

namespace Drx.Sdk.Network.Http.Asp
{
    /// <summary>
    ...
    public class DrxHttpAspServer : IDisposable
    {
        ...
        private readonly Func<Drx.Sdk.Network.Http.Protocol.HttpRequest, Task<Drx.Sdk.Network.Http.Protocol.HttpResponse>>? _requestHandler;
        ...
        public DrxHttpAspServer(int port = 5000, Action<IApplicationBuilder>? configureApp = null, Func<Drx.Sdk.Network.Http.Protocol.HttpRequest, Task<Drx.Sdk.Network.Http.Protocol.HttpResponse>>? requestHandler = null, ILoggerFactory? loggerFactory = null)
        {
            _port = port;
        ...
        public async Task StartAsync()
        {
            ...
            if (_requestHandler != null)
            {
                app.Map("/{**catchall}", async ctx =>
                {
                    ...
                    {
                        // 处理异常，返回 500
                        resp = new HttpResponse(500, ex.ToString());
                    }
        ...
        private static async Task<Drx.Sdk.Network.Http.Protocol.HttpRequest> ToFrameworkRequestAsync(HttpContext ctx)
        {
            var fr = new HttpRequest();
            ...
            try
            {
                ...
                fr.ClientAddress = HttpRequest.Address.FromEndPoint(fr.RemoteEndPoint, fr.Headers);
            }
            catch { }
        ...
        private static async Task ApplyFrameworkResponseAsync(HttpContext ctx, Drx.Sdk.Network.Http.Protocol.HttpResponse resp)
        {
            if (resp == null)
            {
