...
## Auth App 管理（服务端管理员 API）

服务端内置了 Auth App 的运行时管理端点，可通过 HTTP API 动态创建、更新和查询已注册的 AuthApp，无需重启服务器。

> **权限要求**：管理端点（列表、保存）仅限 `UserPermissionGroup.System`（值 = 0）的用户访问；快速授权 URL 端点公开可用。
...
返回所有已注册/已启用的 Auth App 信息（脱敏，不含完整 clientSecret）。
...
- `id` 为空 → 创建新 App（使用 `RegisterAuthApp`）
- `id` 非空 → 更新已有记录（`clientSecret` 为空时不修改密钥）

### GET /api/oauth/apps/quick-login-url

查询参数：

| 参数 | 必填 | 说明 |
|------|------|------|
| `clientId` | 是 | AuthApp 的 clientId |
| `scope` | 否 | 权限范围，默认 `profile` |

响应：

```json
{
  "authorizeUrl": "https://auth.example.com/api/oauth/authorize?client_id=demo_client&..."
}
```
