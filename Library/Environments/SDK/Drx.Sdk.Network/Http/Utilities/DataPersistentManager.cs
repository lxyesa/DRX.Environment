...
namespace Drx.Sdk.Network.Http.Utilities
{
    ...
    /// 并支持把某个分组持久化为 {id}.db（使用 SqliteV2<T> 实现）。
    /// </summary>
    internal class DataPersistentManager
    {
        ...
        /// 实现：通过反射构造 SqliteV2&lt;T&gt;，先 ClearAsync，然后逐条 Push。
        /// 此方法为同步阻塞调用（内部会等待异步方法完成），若需要异步保存请自行封装为 Task.Run。
        /// </summary>
        /// <param name="id">分组标识</param>
        /// <param name="basePath">可选：存储基路径（传入 null 则使用当前工作目录或 SqliteV2 的默认行为）</param>
        ...
        /// 实现：通过 SqliteV2<T>.GetAllAsync 读取并检查是否有元素。
        /// </summary>
        public async Task<bool> ExistsAsync<T>(string id, string? basePath = null, CancellationToken ct = default) where T : Models.DataModelBase, new()
        {
            ...
            {
                // 分组内存不存在，先检查磁盘上是否存在 db 文件，避免构造 SqliteV2 导致创建空文件
                ...
                try
                {
                    var sqlite = new SqliteV2<T>(dbFileName, basePath);
                    var all = await sqlite.SelectAllAsync(ct).ConfigureAwait(false);
        ...
        /// 实现：使用 SqliteV2<T>.GetAllAsync 读取所有实体，然后用锁替换内存桶中的集合内容。
        /// </summary>
        /// <typeparam name="T">实体类型，需继承 Models.DataModelBase 且有无参构造</typeparam>
        /// <param name="id">分组标识</param>
        /// <param name="basePath">可选基路径</param>
        ...
        public async Task LoadAsync<T>(string id, string? basePath = null, CancellationToken ct = default) where T : Models.DataModelBase, new()
        {
            ...
            if (!File.Exists(fullPath))
            {
                // 确保创建空的内存桶但不触发 SqliteV2 的构造
                _storage.GetOrAdd(id, _ => new DataBucket(typeof(T)));
                return;
            }

            // 构造 SqliteV2<T> 并读取所有数据
            var sqlite = new SqliteV2<T>(dbFileName, basePath);

            List<T> items;
            try
            {
                items = await sqlite.SelectAllAsync(ct).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"从数据库加载 SqliteV2<{typeof(T).Name}> 数据失败: {ex.Message}", ex);
            }
        ...
        public async Task SaveAsync(string id, string? basePath = null, CancellationToken ct = default)
        {
            ...
            {
                throw new InvalidOperationException($"创建 SqliteV2<{elementType.Name}> 实例失败: {ex.Message}", ex);
            }
            ...
            {
                throw new InvalidOperationException($"未在 SqliteV2<{elementType.Name}> 中找到 Push 方法");
            }
        ...
        {
            var sqliteGeneric = typeof(SqliteV2<>).MakeGenericType(elementType);
            ...
            // Push 方法
            entry.PushMethod = sqliteGeneric.GetMethod("Insert", new Type[] { elementType });
