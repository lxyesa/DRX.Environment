...
## 主要方法
- `Push(T entity)` / `PushAsync(T entity, CancellationToken)`：插入或更新主表及所有关联表数据
- `Query(string propertyName, object value)` / `QueryAsync(...)`：按属性查询
- `QueryById(int id)` / `QueryByIdAsync(int id)`：按主键查询
- `Update(T entity)` / `UpdateAsync(T entity)`：更新主表及所有关联表
- `Delete(int id)` / `DeleteAsync(int id)`：删除主表及所有关联表数据
- `GetAll()` / `GetAllAsync()`：获取所有主表数据及其关联表

---

## 底层机制与设计要点
- 支持属性类型为 `IDataTable`（一对一）或 `List<IDataTable>`（一对多）自动识别
- 所有操作自动开启事务，保证数据一致性（同步/异步均如此）
- 自动创建和维护主表及所有关联表结构，无需手动建表
...
- 主键 Id 必须为 int 类型
- 关联表属性必须为 `IDataTable` 或 `List<IDataTable>`
- 不支持多级嵌套关联（即子表不能再有子表）
...
**Q: 如何支持多表关联？**
A: 只需在主表类中添加多个 `IDataTable` 或 `List<IDataTable>` 属性即可。
