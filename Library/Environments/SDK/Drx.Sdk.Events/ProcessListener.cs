    ...
    public class ProcessListener : IDisposable
    {
        ...
        private async Task MonitorProcessesAsync()
        {
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                {
                    // 记录意外异常但继续监控
                    System.Diagnostics.Debug.WriteLine($"进程监控异常: {ex.Message}");
                }
            }
        }
        ...
        private void CheckProcessChanges()
        {
            try
            {
                ...
                // 检查已关闭的进程
                List<int> stoppedIds = new List<int>();
                foreach (var kvp in runningProcesses)
                {
                    int id = kvp.Key;
                    if (!currentProcessDict.ContainsKey(id))
                    {
                        // 进程已停止
            ...
            {
                System.Diagnostics.Debug.WriteLine($"检查进程变化时出错: {ex.Message}");
            }
        }
        ...
        private void UpdateRunningProcesses()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"更新进程列表时出错: {ex.Message}");
            }
        }
