...
namespace Drx.Sdk.Shared
{
    ...
    public static class Logger
    {
        ...
        /// <summary>
        /// 以信息级别记录一条日志（同步，等待输出完成）。
        /// </summary>
        /// <param name="header">日志头部/来源。</param>
        /// <param name="message">日志正文。</param>
        /// <param name="lineNumber">调用处行号，自动填充。</param>
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        public static void Info(string header, string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Info, header, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以信息级别记录一条日志（异步，批量输出）。
        /// </summary>
        public static Task InfoAsync(string header, string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Info, header, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以信息级别记录一条日志（同步，Text对象）。
        /// </summary>
        public static void Info(string header, Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Info, header, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以信息级别记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task InfoAsync(string header, Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Info, header, null, text, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(level, header, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以指定级别记录一条日志（异步）。
        /// </summary>
        public static Task LogAsync(LogLevel level, string header, string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(level, header, message, null, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(level, header, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以指定级别记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task LogAsync(LogLevel level, string header, Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(level, header, null, text, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Fail, className, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以错误级别记录一条日志（异步）。
        /// </summary>
        public static Task ErrorAsync(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Fail, className, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以错误级别记录一条日志（同步，Text对象）。
        /// </summary>
        public static void Error(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Fail, className, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以错误级别记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task ErrorAsync(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Fail, className, null, text, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Dbug, className, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以调试级别记录一条日志（异步）。
        /// </summary>
        public static Task DebugAsync(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Dbug, className, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以调试级别记录一条日志（同步，Text对象）。
        /// </summary>
        public static void Debug(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Dbug, className, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以调试级别记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task DebugAsync(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Dbug, className, null, text, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Warn, className, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以警告级别记录一条日志（异步）。
        /// </summary>
        public static Task WarnAsync(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Warn, className, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以警告级别记录一条日志（同步，Text对象）。
        /// </summary>
        public static void Warn(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Warn, className, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以警告级别记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task WarnAsync(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Warn, className, null, text, className, lineNumber, filePath);
        }
        ...
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Info, className, message, null, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以信息级别记录一条日志（异步，简化重载）。
        /// </summary>
        public static Task InfoAsync(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Info, className, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以信息级别记录一条日志（同步，Text对象，简化重载）。
        /// </summary>
        public static void Info(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            EnqueueSync(LogLevel.Info, className, null, text, className, lineNumber, filePath);
        }

        /// <summary>
        /// 以信息级别记录一条日志（异步，Text对象，简化重载）。
        /// </summary>
        public static Task InfoAsync(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Info, className, null, text, className, lineNumber, filePath);
        }
        ...
        /// <summary>
        /// 以 Trace（映射到调试级别）记录一条日志（异步）。
        /// </summary>
        public static Task TraceAsync(string message, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Dbug, className, message, null, className, lineNumber, filePath);
        }
        ...
        /// <param name="filePath">调用处文件路径，自动填充。</param>
        /// <summary>
        /// 以 Trace（映射到调试级别）记录一条日志（同步，Text对象）。
        /// </summary>
        ...
        /// <summary>
        /// 以 Trace（映射到调试级别）记录一条日志（异步，Text对象）。
        /// </summary>
        public static Task TraceAsync(Text text, [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "")
        {
            var className = GetShortClassName(filePath);
            return EnqueueAsync(LogLevel.Dbug, className, null, text, className, lineNumber, filePath);
        }
        ...
        /// 异步入队日志（批量输出）。
        /// </summary>
        private static Task EnqueueAsync(LogLevel level, string header, string? message, Text? text, string className, int lineNumber, string filePath)
        {
            if (string.IsNullOrEmpty(message) && text == null) return Task.CompletedTask;
            string moduleInfo = "";
        ...
        /// 同步写入日志（立即输出到控制台）。
        /// </summary>
        private static void EnqueueSync(LogLevel level, string header, string? message, Text? text, string className, int lineNumber, string filePath)
        {
        ...
        private static void LogFatalException(string source, Exception ex)
        {
            try
            {
                ...
                EnqueueSync(LogLevel.Fatal, header, message, null, header, 0, "");
            }
            catch
            {
                try
