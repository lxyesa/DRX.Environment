...
public abstract class DRXServer : DRXBehaviour
{
    ...
    public virtual void BanneClient(DRXSocket clientSocket, int time_h)
    {
        ...
        var path = File.BanPath;

        // 设置封禁时间
        client.Ban(DateTime.Now.AddHours(time_h));

        // 写入封禁信息到文件
        _ = File.WriteJsonKeyAsync(path, client.UID, client.UnBandedDate).ConfigureAwait(false);

        client.SaveToFile(File.UserPath);
    ...
    public virtual void UnBanneClient(DRXSocket clientSocket)
    {
        ...
        // 移除封禁信息
        _ = File.RemoveJsonKeyAsync(path, client.UID).ConfigureAwait(false);
    ...
    public virtual void BeginReceiveCommand()
    {
        OnDataReceived += (sender, args) =>
        {
            ...
            var packet = BasePacket.Unpack<DRXPacket>(args.Packet, _key);
    ...
    protected virtual void ProcessReceivedData(DRXSocket clientSocket, byte[] data)
    {
        ...
        {
            // 解析收到的数据包
            BasePacket receivedPacket = BasePacket.Unpack<DRXPacket>(data, _key);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Obsolete("此方法已弃用，请使用新的基于 BasePacket 的方法。")]
    public virtual void Send<T>(DRXSocket clientSocket, T packet, string key) where T : BasePacket
    {
        try
        {
            if (!ValidateClientForSend(clientSocket)) return;
    ...
    public virtual void Send<T>(DRXSocket clientSocket, T originPacket, T responsePacket, string key) where T : BasePacket
    {
        try
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public virtual async Task<byte[]> SendAsync<T>(DRXSocket clientSocket, T packet, string key, int timeout = 0) where T : BasePacket
    {
        try
        {
            if (!ValidateClientForSend(clientSocket))
