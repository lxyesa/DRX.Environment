...
namespace NetworkCoreStandard.Common
{
    ...
    public class DRXClient : DRXBehaviour
    {
        ...
        /// <summary>
        /// 发送基于 <see cref="BasePacket"/> 的数据包，并等待响应。
        /// </summary>
        /// <typeparam name="T">数据包类型，必须继承自 <see cref="BasePacket"/>。</typeparam>
        /// <param name="packet">要发送的数据包。</param>
        /// <param name="timeout">等待响应的超时时间（毫秒）。</param>
        /// <returns>服务器响应的数据包字节数组。</returns>
        public virtual async Task<byte[]> SendAsync<T>(T packet, int timeout = 0) where T : BasePacket
        {
            if (!_isConnected)
            {
                OnErrorCallback?.Invoke(this, new NetworkEventArgs(
        ...
        protected virtual void ReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    BasePacket receivedPacket = BasePacket.Unpack<DRXPacket>(buffer.Take(bytesRead).ToArray(), _key);
